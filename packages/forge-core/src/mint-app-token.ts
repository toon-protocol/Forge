 ...
 *
 * App installation tokens expire ONE HOUR after mint. A runner that mints
 * once at job start and pushes only at the very end of a long cycle dies at
 * the final `git push` on any run that crosses that hour — after every
 * expensive agent iteration has already been spent (connector#462,
 * diagnosed live on connector#459). This module signs the App JWT and
 * exchanges it for an installation token itself (RS256 via `node:crypto` —
 * no `jsonwebtoken` dependency), so a caller can mint immediately before
 * EVERY push instead of once per run.
 *
 * `createTokenMinter` returns `undefined` when `APP_ID`/`APP_PRIVATE_KEY`
 * are not set on the host, so a caller's local-dev / ambient-`GH_TOKEN` path
 * is unchanged — that fallback is the caller's responsibility (see
 * `sandcastle-runners.ts`'s `pushBranch`).
 */
 ...
 * leaves room for clock drift between this host and GitHub's. Exported for
 * direct unit testing (decode + verify against a matching public key).
 */
 ...
 * when the App credentials are absent from the host — the signal a caller
...
export function createTokenMinter(
  ...
  overrides: Partial<Pick<MintAppTokenConfig, 'fetchFn' | 'now'>> = {}
  ...
  return () => mintInstallationToken({ appId, privateKey, repo, ...overrides });
}
